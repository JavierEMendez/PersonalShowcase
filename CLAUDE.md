...
8. Port the internal buyer's model (the workbook inventoried in `private/multifamily-model-inventory.md`) sheet by sheet into `core/copilot/` with typed inputs mirroring the workbook's input cells and modules in the workbook's calculation order; reconcile to the workbook's own values on the internal deal with a private script (never committed), zero mismatches; then reseed with Sawyer Bend. The structural engine from step 4 is replaced. Nothing from the workbook (figures, names, addresses, defined names) enters the repo.
9. Re-point Screen, Underwrite and the memo at the ported model: input tabs that mirror the workbook's sheets (as the MPC tool's Tract, Costs, Revenue and Lookups tabs do), the intake filling those inputs from the OM, rent roll and T-12, the memo facts from the ported outputs.
10. README rewritten as an IC memo: outcome first, GIF demo, "how to evaluate this in five minutes", architecture, decision log link. Case study in `docs/`.
